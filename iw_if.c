...
/**
 * check that iface_name does exists and is up
 */
 void iface_exists_up(char *iface_name) {
	struct interface_info *head = NULL, *cur;
	struct if_info net_info;
	bool exists = false;
	int count;

	//get available interfaces list
	iw_nl80211_get_interface_list(&head);
	count = count_interface_list(head);
	if (!count) err_quit("ERROR: No wireless interfaces found!");

	//check for given iface
	for (cur = head; cur; cur = cur->next) {
		if(!strcmp(iface_name, cur->ifname)) {
			exists = true;
			break;
		}
	}
	if(!exists) err_quit("ERROR: No wireless interface %s found!", iface_name);

	//check whether iface is up or not
	if_getinf(iface_name, &net_info);
	if(!(net_info.flags & IFF_UP)) err_quit("ERROR: Wireless interface %s is down!", iface_name);
}